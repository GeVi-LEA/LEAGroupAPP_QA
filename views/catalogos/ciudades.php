...
<span id="valor" hidden>Ciudad</span>
...
<header class="header">
    <div class="row">
        ...
        <div class="col-6 text-center"><h1 class="titulo">Ciudades</h1></div>
...
<nav class="menu">
    ...
    <span> <a href="<?= catalogosUrl ?>?controller=Catalogo&action=showEstados">Estados</span></a>
    <span id="mostrarForm"> Agregar ciudad</span>
</nav>
<section id="secForm">
    <form action="<?= catalogosUrl ?>?controller=Catalogo&action=saveCiudad" method="post" class="formulario" id="formularioCiudad" >
        ...
        <div class="row p-1">
            ...
            <div class="col-9 d-flex justify-content-between">
                <input type="text" name="nombre" class="inputMedium capitalize" id="nombre" maxlength="30" placeholder="Ej. Monterrey"/>          
            </div>
        </div>
        <div class="row p-1 ">
            <div class="col-3 text-right">
            ...
            <div class="col-9">
                <input type="text" name="clave" class="inputSmall text-uppercase" id="clave" maxlength="7" placeholder="Ej. Mty"/>
            </div>
        </div>
        <div class="row p-1 ">
            <div class="col-3 text-right">
            ...
            <div class="col-9">
                <select name="estado" class="inputMedium" id="estado"> 
                    <option value="" selected disabled>--Selecciona--</option>
                    <?php
                    if (!empty($estados)):
                        foreach ($estados as $e):
                            ?>
                            <option value="<?= $e->id ?>"><?= $e->nombre . ", " ?><?= $e->clave_pais ?></option>
                        <?php
                        endforeach;
                    endif;
                    ?>
                ...
                <div >
                    <ul class="error text-left" id="error">
                        <?php if (isset($_SESSION['errores']['nombre']) && $_SESSION['errores']['nombre'] == 'invalid'): ?>
                            <li>El <b>nombre</b> de Ciudad ya existe</li>                   
                        <?php endif; ?>
                        <?php if (isset($_SESSION['errores']['clave']) && $_SESSION['errores']['clave'] == 'invalid'): ?>
                            <li>La <b>clave</b> de Ciudad ya existe</li>                
                            <?php
                        endif;
...
<section class="sec-tabla text-center table-responsive-sm" id="seccionCiudad">
<?php if (!empty($ciudades)): ?>

        <table class="table table-condensed" id="tablaCiudad">
            <thead>
            <th>Ciudad</th>
            <th>Clave</th>
            <th>Estado</th>
    ...
    <?php foreach ($ciudades as $ciu): ?>
                    <tr class="tr">
                        <td id="idTabla" hidden><?= $ciu->id; ?></td>
                        <td id="nombreTabla"><?= $ciu->nombre; ?></td>
                        <td id="claveTabla"><?= $ciu->clave; ?></td>
                        <td id="estadoIdTabla" hidden><?= $ciu->estado_id; ?></td>
                        <td id="estadoTabla"><?= $ciu->nombre_estado; ?></td>
                        <td id="paisdTabla"><?= $ciu->nombre_pais; ?></td>
                        <td> 
                            <div>
                                <a ><span id="edit" class="material-icons i-edit" title="Editar">edit</span></a>                    
                                <a id="linkDelete" href="<?= catalogosUrl ?>?controller=Catalogo&action=deleteCiudad&id=<?= $ciu->id; ?>" ></a><span id="delete" class="material-icons i-delete" title="Eliminar">delete_forever</span>
                            </div>
                        </td>
        <?php endforeach; ?>
            </tbody>
