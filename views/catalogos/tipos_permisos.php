...
<span id="valor" hidden>TipoPermisos</span>
...
<header class="header">
    <div class="row">
        <div class="col-1 text-center"><a href="<?= catalogosUrl ?>?controller=Catalogo&action=showUsuarios"><span class="material-icons p-1 i-apps" id="i-apps" title="Usuarios">keyboard_backspace</span></a></div>
        <div class="col-10 text-center"><h1 class="titulo">Permisos</h1></div>
        <div class="col-1"><a><span class="material-icons p-1 i-close" id="i-close" title="Cerrar">cancel</span></a></div>
    </div>
</header>
<nav class="menu">
    <span id="mostrarForm">Agregar Permiso</span>
</nav>
<section id="secForm">
    <form action="<?= catalogosUrl ?>?controller=Catalogo&action=saveTipoPermiso" method="post" class="formulario" id="formularioTipoPermisos" >
        ...
        <div class="row p-1">
            ...
            <div class="col-9 d-flex justify-content-between">
                <input type="text" name="nombre" class="inputMedium capitalize" id="nombre" maxlength="15" placeholder="Ej. Sistemas"/>           
            </div>
        </div>
        <div class="row p-1 ">
            <div class="col-3 text-right">
                ...
                <div >
                    <ul class="error text-left" id="error">
                        <?php if (isset($_SESSION['errores']['nombre']) && $_SESSION['errores']['nombre'] == 'invalid'): ?>
                            <li>El <b>nombre</b> de permiso ya existe</li>                   
                        <?php endif; ?>
                        <?php if (isset($_SESSION['errores']['clave']) && $_SESSION['errores']['clave'] == 'invalid'): ?>
                            <li>La <b>clave</b> de permiso ya existe</li>                
                            <?php
                        endif;
...
<section class="sec-tabla text-center table-responsive-sm" id="seccionTipoPermisos">
    <table class="table table-condensed" id="tablaTipoPermisos">
        <?php if (!empty($permisos)): ?>
            ...
            <tbody>
                <?php foreach ($permisos as $p): ?>
                    <tr class="tr">
                        <td id="idTabla"><?=$p->id; ?></td>
                        <td id="nombreTabla"><?= $p->nombre; ?></td>
                        <td id="claveTabla"><?= $p->clave; ?></td>
                        <td id="descripcionTabla"><?= $p->descripcion; ?></td>
                        <td> 
                            <div>
                                <a ><span id="edit" class="material-icons i-edit" title="Editar">edit</span></a>                    
                                <a id="linkDelete" href="<?= catalogosUrl ?>?controller=Catalogo&action=deleteTipoPermiso&id=<?= $p->id; ?>" ></a><span id="delete" class="material-icons i-delete" title="Eliminar">delete_forever</span>
                            </div>
                        </td>
                    <?php endforeach; ?>
            </tbody>
